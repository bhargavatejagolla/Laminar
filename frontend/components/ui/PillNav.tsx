...
const PillNav = ({
  ...
  useEffect(() => {
    ...
    // Using a microtask delay instead of document.fonts.ready for more robust initial layout
  ...
  const renderLink = (item: NavItem, i: number, isMobile = false) => {
    const classNameBase = isMobile ? 'mobile-menu-link' : 'pill';
    const activeClass = currentActiveHref === item.href ? ' is-active' : '';
    ...
    if (isAnchorLink(item.href)) {
      return (
        <a
          ...
          onMouseLeave={isMobile ? undefined : () => handleLeave(i)}
  ...
  return (
    <div className={`pill-nav-container ${className}`} style={{ position: "relative" }}>
      <nav className="pill-nav" aria-label={t("auto.Primary_4134") || "Primary"} style={cssVars}>
        ...
        >
          {logoComponent ? logoComponent : <img src={logo} alt={logoAlt} ref={logoImgRef} />}
        </Link>
