            ...
            <div className="flex-1 overflow-y-auto p-6 lg:p-10 z-10 custom-scrollbar relative">
                ...
                <header className="mb-10 flex flex-col items-center lg:items-start border-b border-red-900/30 pb-6 mt-12 lg:mt-0 lg:pl-28">
                    <div className="flex items-center gap-4 mb-2">
                        ...
                        <div>
                            <h1 className="text-3xl font-black uppercase tracking-widest text-white">{t("auto.ActiveMissingPe_3998") || "Active Missing Persons"}</h1>
                            <p className="text-red-400/80 uppercase tracking-widest text-xs font-mono">{t("auto.LaminarGlobalNe_4308") || "Laminar Global Network Watchlist"}</p>
                        </div>
                    </div>
                </header>
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-red-500/30">
                        ...
                        <h2 className="text-xl font-bold uppercase tracking-widest text-red-500/50">{t("auto.NetworkClear_3978") || "Network Clear"}</h2>
                        <p className="text-sm font-mono mt-2">{t("auto.Noactivemissing_8352") || "No active missing person reports."}</p>
                    </div>
                ) : (
                            ...
                            <div key={c.id} className="bg-red-950/20 border border-red-900/50 rounded-2xl overflow-hidden flex flex-col hover:border-red-500/50 transition-colors">
                                <div className="h-48 bg-black/60 relative flex items-center justify-center border-b border-red-900/50">
                                    ...
                                    <div className="absolute top-3 right-3 bg-red-600 text-white text-[10px] font-black uppercase px-2 py-1 rounded shadow-[0_0_10px_rgba(239,68,68,0.8)] animate-pulse">
                                        {t("auto.ACTIVESEARCH_5837") || "ACTIVE SEARCH"}
                                    </div>
                                    {c.match_found && (
                                        <div className="absolute bottom-3 left-3 bg-red-500/20 text-red-400 border border-red-500/50 text-[10px] font-bold uppercase px-2 py-1 rounded backdrop-blur-md flex items-center gap-1">
                                            <Camera className="w-3 h-3" /> {t("auto.AIMatched_4040") || "AI Matched"}
                                        </div>
                                    )}
                                </div>
                                <div className="p-5">
                                    ...
                                    <div className="flex gap-2 mt-6">
                                        ...
                                        >
                                            {t("auto.JoinSearchFeed_463") || "Join Search Feed"}
                                        </a>
                                        <button
                                            onClick={async () => {
                                                try {
            ...
            <aside className="w-full lg:w-[400px] xl:w-[480px] bg-[#0a0000] border-t lg:border-t-0 lg:border-l border-red-900/50 flex flex-col z-20 shrink-0">
                <div className="p-6 border-b border-red-900/30 bg-red-950/10">
                    <h2 className="text-xl font-black uppercase tracking-widest text-red-500 flex items-center gap-2">
                        ...
                        {t("auto.ReportMissing_2952") || "Report Missing"}
                    </h2>
                    <p className="text-xs text-red-500/60 font-mono mt-1">{t("auto.AINetworkDispat_8212") || "AI Network Dispatch Portal"}</p>
                </div>
                    ...
                    {submitted ? (
                        <div className="h-full flex flex-col gap-6 text-left pb-10">
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-[#0f0000] border border-red-500/40 rounded-xl p-4 shadow-[0_0_20px_rgba(239,68,68,0.2)]">
                                <div className="flex items-center justify-between mb-3">
                                    <h3 className="text-sm font-black uppercase text-red-500 tracking-widest flex items-center gap-2">
                                        ...
                                        {t("auto.LiveSearchStatu_4942") || "Live Search Status"}
                                    </h3>
                                    <span className="text-[10px] font-mono text-red-400 border border-red-500/30 px-2 py-0.5 rounded bg-red-950/30">{t("auto.ACTIVE_6722") || "ACTIVE"}</span>
                                </div>
                                <div className="grid grid-cols-2 gap-3 text-xs font-mono">
                                    <div className="bg-black/50 p-2 rounded border border-red-900/30">
                                        <p className="text-red-500/50 mb-1">{t("auto.CamerasScanned_532") || "Cameras Scanned"}</p>
                                        <p className="text-lg font-black text-red-400">1,242</p>
                                    </div>
                                    <div className="bg-black/50 p-2 rounded border border-red-900/30">
                                        <p className="text-red-500/50 mb-1">{t("auto.MatchesFound_3991") || "Matches Found"}</p>
                                    ...
                                    <div className="col-span-2 bg-red-950/20 p-2 rounded border border-red-500/20 flex justify-between items-center">
                                        <p className="text-red-400/80">{t("auto.HighestConfiden_6567") || "Highest Confidence"}</p>
                            ...
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-[#0a0000] border border-red-900/50 rounded-xl p-4">
                                <h3 className="text-[11px] font-black uppercase text-red-500 tracking-widest mb-3">{t("auto.NetworkReach_6628") || "Network Reach"}</h3>
                                <div className="flex justify-between items-end border-b border-red-900/30 pb-2 mb-2">
                                    <span className="text-xs text-red-100/50">{t("auto.ConnectedCamera_8680") || "Connected Cameras"}</span>
                                    <span className="text-sm font-mono font-bold text-white">127</span>
                                </div>
                                <div className="flex justify-between items-end border-b border-red-900/30 pb-2 mb-2">
                                    <span className="text-xs text-red-100/50">{t("auto.ActiveNodes_6260") || "Active Nodes"}</span>
                                    <span className="text-sm font-mono font-bold text-white">23</span>
                                </div>
                                <div className="flex justify-between items-end">
                                    <span className="text-xs text-red-100/50">{t("auto.SearchRange_1501") || "Search Range"}</span>
                            ...
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-[#0f0000] border border-red-500/30 rounded-xl p-4">
                                <h3 className="text-[11px] font-black uppercase text-red-500 tracking-widest mb-3">{t("auto.AutoEscalation_9486") || "Auto Escalation"}</h3>
                                <div className="bg-red-500/10 border border-red-500/30 p-3 rounded-lg mb-3">
                                    <p className="text-center text-sm font-black text-white uppercase tracking-widest mb-1">{t("auto.MissingPersonLo_6519") || "Missing Person Located"}</p>
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className="flex items-center gap-2 text-xs text-white bg-black/50 p-2 rounded"><CheckCircle className="w-3 h-3 text-red-500" /> {t("auto.SecurityTeams_3352") || "Security Teams"}</div>
                                    <div className="flex items-center gap-2 text-xs text-white bg-black/50 p-2 rounded"><CheckCircle className="w-3 h-3 text-red-500" /> {t("auto.LocalPolice_9262") || "Local Police"}</div>
                            ...
                            <div className="flex flex-col gap-3 mt-4">
                                ...
                                >
                                    {t("auto.JoinLiveRecover_1477") || "Join Live Recovery Feed"}
                                </a>
                                <button 
                                    onClick={() => {
                                        setSubmitted(false);
                                ...
                                >
                                    {t("auto.FileAnotherRepo_329") || "File Another Report"}
                                </button>
                            </div>
                        </div>
                    ) : (
                        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                                ...
                                >
                                    ...
                                    {previewUrl ? (
                                        ...
                                        <>
                                            ...
                                            <span className="text-red-400/60 font-medium text-xs">{t("auto.Taptouploadphot_5021") || "Tap to upload photo"}</span>
                                        </>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="bg-black/40 border border-red-900/30 rounded-xl p-5 flex flex-col gap-4">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        placeholder={t("auto.MissingPersonsN_1210") || "Missing Person's Name"} 
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        placeholder={t("auto.LastSeenLocatio_1815") || "Last Seen Location"} 
                                        className="w-full bg-red-950/20 border border-red-900/40 rounded-lg py-3 pl-10 pr-3 text-sm text-white placeholder-red-500/30 focus:outline-none focus:border-red-500"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="bg-black/40 border border-red-900/30 rounded-xl p-5 flex flex-col gap-4">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        placeholder={t("auto.YourName_6614") || "Your Name"} 
                                        className="w-full bg-red-950/20 border border-red-900/40 rounded-lg py-3 pl-10 pr-3 text-sm text-white placeholder-red-500/30 focus:outline-none focus:border-red-500"
                                    />
                                </div>
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        placeholder={t("auto.YourPhoneNumber_2294") || "Your Phone Number"} 
                                        className="w-full bg-red-950/20 border border-red-900/40 rounded-lg py-3 pl-10 pr-3 text-sm text-white placeholder-red-500/30 focus:outline-none focus:border-red-500"
                                    />
                                </div>
                            </div>
                            ...
                            >
                                {isSubmitting ? (
                                    <span className="animate-pulse">{t("auto.Transmitting_8588") || "Transmitting..."}</span>
                                ) : (
                                    <>
                                        <Send className="w-4 h-4" />
                                        {t("auto.TransmitSOS_4651") || "Transmit SOS"}
                                    </>
                                )}
                            </button>
                        </form>
