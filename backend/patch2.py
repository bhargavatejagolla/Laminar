...
for i, line in enumerate(lines):
    ...
    # 4. Remove old heartbeat try block and add retry block
    # We'll just replace the try/except block. 
    # Let's detect the line 'cam_row.update_heartbeat()' and replace its outer block.
    # A cleaner way is using string replace on the full text with normalized newlines.
...
# Replace the heartbeat block using regex targeting exactly the block
import re
